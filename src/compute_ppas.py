...
# -------------------------
def perceptual_phase_similarity(
    x_ref,
    x_deg,
    sr,
    ...
    mags_ref = np.abs(S_ref)
    mags_deg = np.abs(S_deg)
    ...
    E_ref = np.abs(S_ref_mel)
    E_deg = np.abs(S_deg_mel)
    ...
    for b in range(n_mels):
        ...
        if idx.size == 0:
            env_corr[b, :] = 0.0
            continue
        r = E_ref_smooth[b, idx]
        d = E_deg_smooth[b, idx]
...
def align_and_compute_ppas(
    ...
    for idx in np.arange(x_ref.ndim):
        ...
        if verbose:
            print(
                f"GCC-PHAT coarse shift (samples) for audio track #{idx}: {shift_samples:.3f} -> {shift_samples/sr_target*1000:.3f} ms"
            )
    ...
    for idx in np.arange(x_ref.ndim):
        ...
        if err == 0:
            ...
            # PPAS compute
            #
            ppas = perceptual_phase_similarity(
                x_ref_final,
                x_deg_final,
                sr_target,
                n_fft=fft_size,
                hop_length=fft_overlap,
                n_mels=_PPAS_DEFAULT_N_MELS,
                alpha=_PPAS_DEFAULT_ALPHA,
                env_corr_window_frames=_PPAS_DEFAULT_ENVELOPE_CORRELATION_WINDOW_FRAMES,
...
if __name__ == "__main__":
    ...
    ref_aligned, deg_aligned, ppas, gcc_phat_shift, gcc_phat_delta_shift = align_and_compute_ppas(
        ref_file, deg_file, sr_target=16000, max_global_shift_s=0.02, do_dtw_fallback=True
    )
